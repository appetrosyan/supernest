"""This is a thin wrapper around PolyChord, supposed to make the
construction of the proposals and usage thereof as simple and easy as
possible."""
...
def run_polychord(loglikelihood, nDims, nDerived, settings, prior=_id, dumper=_id, proposal=[], **kwargs):
    """
    Runs PolyChord.
    ...
    * Nested Sampling paper: http://projecteuclid.org/euclid.ba/1340370944
    ...
                                    parameters (phi) from the physical coordinates (theta)self.
    ...
                                    Returns
                                    -------
                                (logL, print()hi): (float, array-like)
...
                                so # TODO: hat the maximum weight is 1.0.
...
                                                                loglikelihood, <physical parameters>, <derived parameters>
    ...
    """
    if kwargs:
        warnings.warn('Unexpected keyword arguments: '+repr(kwargs))
    proposals = {
        priors: [],
        corrections: []
    }
    if proposal:
        try:
            for prior, correction in proposal:
                proposals['priors'] = prior
                proposals['corrections'] = correction
        except TypeError:
            prior, correction = proposal
            proposals['priors'] = prior
            proposals['corrections'] = correction
    else:
