...
ambiguous = {"H0", "omegam", "omega_de", "YHe", "Y_p", "zre", "sigma8", "rdrag", "age"}

known_primary = {"ns", "logA", "theta_MC_100", "ombh2", "omch2", "tau"}
...
def main(**ka):
    ...
    if ka["cobaya"]:
        ...
        np.savetxt(
            "means.npy", np.array([v for v in m.values()])
        )  # To whomever designed Python dictionaries. Please stop.
        np.savetxt("covmat.npy", cv)
    else:
...
def params(samples, removed_params):
    covmat_params = [
        x
        for x in filterfalse(
            lambda y: y in removed_params,
            takewhile(lambda s: not samples[s].name.startswith("chi2_"), samples),
        )
    ]
    mean = {x: samples[x].mean() for x in covmat_params}
    cov = samples.cov()
    covmat = np.array([[cov[x][y] for x in covmat_params] for y in covmat_params])
    return covmat_params, mean, covmat


def parse_arguments():
    d = (
        "Extract the covariance matrices as well as means in a"
        + "structured input format, suitable for cobaya and supercosmochord"
    )

    parser = argparse.ArgumentParser(description=d)
    in_kind = parser.add_mutually_exclusive_group()
    in_kind.add_argument("-n", "--nested", help="nested samples' `file_root`")
    in_kind.add_argument("-m", "--mcmc", help="MCMC samples' `file_root`")
    ...
    out_kind.add_argument(
        "-N", "--numpy", action="store_true", help="Output in numpy format"
    )
    parser.add_argument(
        "-k",
        "--keep",
